import { loadData, saveData, exportData, importData, resetData } from './storage-v1.3.0.js?v=1.3.0';
import { uid, esc, fmtDate, reputationLabel, reputationPercent, hoursUntil, activeGraffitis, getNextTurn, countdownText, territoryState, recommendation } from './utils-v1.3.0.js?v=1.3.0';
...
function territoryPin(t){
  ...
  return `<button class="territory-pin ${selectedTerritoryId===t.id?'selected':''} ${state.key==='risk'?'risk':''}" data-territory="${t.id}" style="left:${t.x}%;top:${t.y}%;--pin-color:${color}" aria-label="${esc(t.name)}"><span class="pin-core"><span class="pin-dot"></span></span>${badge}<span class="pin-chip"><b>${esc(t.name)}</b><small>${active}/3</small></span></button>`;
}
