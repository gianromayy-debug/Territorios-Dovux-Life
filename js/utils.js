...
export function territoryState(t,rules){
  ...
  if(t.priority==='ignore') return {key:'ignored',label:'Ignorar'};
  if(active.length<rules.requiredGraffitis) return {key:'risk',label:'En riesgo'};
  if(minH<=rules.dangerHours) return {key:'risk',label:'Crítico'};
  if(minH<=rules.warningHours) return {key:'warning',label:'Atención'};
  return {key:'safe',label:'Seguro'};
}
export function recommendation(data){
  ...
  for(const t of data.territories){
    ...
    const missing=Math.max(0,rules.requiredGraffitis-active.length);
    const priorityScore=t.priority==='maintain'?100:t.priority==='conquer'?45:10;
    const urgency=missing*40+expiring*28+(reputationPercent(t.reputation)/15);
    candidates.push({t,active,missing,expiring,score:priorityScore+urgency});
  }
  candidates.sort((a,b)=>b.score-a.score);
  const picks=[];
  for(const c of candidates){
    let need=Math.max(c.missing,c.expiring);
    if(!need && c.t.priority==='conquer' && c.active.length<rules.requiredGraffitis) need=1;
